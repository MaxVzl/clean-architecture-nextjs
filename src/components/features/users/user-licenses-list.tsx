  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/70 sm:p-5">
      <header className="mb-4 flex items-baseline justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Liste des abonnements logiciels associés à ce membre de l&apos;équipe.
          </p>
        </div>
        {licenses.length > 0 && (
      ...
      {licenses.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-50/60 text-sm dark:border-slate-800 dark:bg-slate-900/60">
              ...
              >
                <div className="col-span-2">
                  ...
                  <p className="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">
                    Facturation {license.billingCycle === "monthly" ? "mensuelle" : "annuelle"}
                  </p>
                </div>
                <div>
                  <p>{license.vendor}</p>
                ...
                <div>
                  <p>
                    {license.monthlyCost.toFixed(2)} {license.currency === "EUR" ? "€" : license.currency}
                  </p>
                </div>
                <div>
                  <span
